...
gc.getEvents = function(calendarShort){
	log.debug("Downloading calendar data for calendar:", calendarShort);
...
gc.onData = function(calendarShort, data){
    ...
    log.debug("Calendar data for", calendarShort, "parsed and loaded");
	if (newChannels.length > 0)	log.info(newChannels.length, " channels added to watch");
}

gc.addChannel = function (channelName) {
	Twitch.addFollow(channelName.toString().toLowerCase());
}
...
gc.processEvent = function(ev){
    ...
    if (ev.location) {
        ...
        if (re.test(ev.location)) {
            var channelName = ev.location.toString().match(re)[1].toLowerCase();
            if (newChannels.indexOf(channelName) == -1) {
                if (!Twitch.isFollowing(channelName)) {
