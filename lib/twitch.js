...
var Twitch = function () {
	...
	//this.twitchStreams = config.otherChannels;
...
t.getFollows = function(start) {
	...
	if (this.tokenValid) {	
		var apiRequest = request({
			url: "https://api.twitch.tv/kraken/users/" + myName + "/follows/channels",
			method: "GET",
			json: true,
			qs: {
				offset: start
...
t.addFollow = function(name) {
	if (this.followsLoaded && !this.isFollowing(name.toString().toLowerCase()) && !this.followsProcessing.includes(name.toString().toLowerCase())) {
		...
		var apiRequest = request({
			method: "PUT",
			url: "https://api.twitch.tv/kraken/users/" + myName + "/follows/channels/" + name.toString().toLowerCase(),
			json: true,
			headers: {
				"Accept": "application/vnd.twitchtv.v3+json",
				"Authorization": 'OAuth ' + this.settings.oAuthToken,
			...
			if (body && body.channel && body.channel.name && !this.isFollowing(body.channel.name.toString().toLowerCase())) {
				log.info("Now following %s",body.channel.name.toString().toLowerCase());
				...
				log.info("Got problem following: %s, %s",body.channel.name, this.isFollowing(body.channel.name));
			}
		});
...
t.isFollowing = function(name) {
	return (this.following.indexOf(name.toString().toLowerCase()) != -1 || (name.toString().toLowerCase() == myName.toString().toLowerCase()));
}
...
t.parseTwitchJSON = function(json) {
		...
		this.following.forEach(function (stream) {
			...
			//If there is new data, we use it.
				...
				if (this.channels.hasOwnProperty(newData.channel.name.toString().toLowerCase())) {
					Models.LastSeen.findOrCreate({ where: { name: newData.channel.name.toString().toLowerCase() }, defaults: { dateTimeSeen: Date.now() } }).spread((foundUser) => {
						foundUser.dateTimeSeen = Date.now();
						foundUser.ChannelId = this.channels[newData.channel.name.toString().toLowerCase()].model.id;
						foundUser.save();
					});
				}
			}
