...
c.clearBarks = function () {
	log.info("Clearing Barks for %s", this.channel.model.name);
	for (var bark in this.barks) {
		if (this.barks.hasOwnProperty(bark)) {
			if (bark.timeout) {
				log.info("ClearBark(%s): Clearing bark %s", this.channel.model.name, bark.command);
...
c.startBark = function (bark) {
		...
		if (ref.from.barks[newBark.message.command]) {
			if (user) {
				...
				try {
					...
					log.error(`Couldn't bark ${newBark.message.message}, ${e.message}. Attempting to remove bark with ${newBark.timeout}...`);
					clearInterval(newBark.timeout);
				}
			}
		} else {
	...
	newBark.intervalBark = function () {
		...
		newBark.timeout = setInterval(newBark.doBark.bind(this), newBark.interval * 60 * 1000, null, newBark);
		newBark.doBark();
	}
...
c.tearDown = function() {
	log.info("Asked to teardown, clearing barks for %s", this.channel.model.name);
	this.clearBarks();
}