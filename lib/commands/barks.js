...
c.addBark = function(user, command, message, isMod, timeout) {
    if (this.checkModLoud(user)) {
        if (this.barks[command]) {
            this.channel.say("Bark " + command + " already exists!");
        } else {
			this.user = user;
...
c.sayBarkAdded = function (command) {
	this.loadBarks();
	this.channel.client.whisper(this.user, this.channel, "Bark " + command + " added!");
...
c.delBark = function (user, command) {
    if (this.checkModLoud(user)) {
        if (!this.barks[command]) {
			this.channel.client.whisper(user, this.channel, "There is no bark for the command " + command);
			//this.channel.say("There is no bark for the command " + command);
        } else {
            var deadId = this.barks[command].message.id;
			this.barks[command].message.destroy();
			this.channel.client.whisper(user, this.channel, "The bark " + command + " has been removed.");
...
c.sayBarkList = function(user){
	...
	this.channel.client.whisper(user, this.channel, "Barks for this channel are: " + barkList);
	//this.channel.say("Barks for this channel are: " + barkList);
}

c.helpBarks = function (user) {
    if (this.checkModLoud(user)) {
		this.channel.client.whisper(user, this.channel, 'Usage: !bark add <command> "<message>" <mod only (true/false)> [interval (optional, in minutes)], !bark remove <command>, !bark list');
