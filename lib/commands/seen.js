        ...
        method: function (match, user, message) {
            ...
            models.LastSeen.findOne({ where: { name: { $like: match[1].toLowerCase() } } }).then(function (foundUser) {
                if (foundUser == null) {
                    ...
                    } else if (foundUser.where == match[1].toLowerCase()) {
                        self.channel.whisper(user, null, 'Last saw ' + foundUser.name + ' ' + Moment(foundUser.dateTimeSeen).fromNow() + ', streaming');
                    } else {
                        self.channel.whisper(user, null, 'Last saw ' + foundUser.name + ' ' + Moment(foundUser.dateTimeSeen).fromNow() + ', in ' + foundUser.where + '\'s room');
                    }
                }
            });
        },
            ...
            models.LastSeen.findOne({ where: { name: { $like: match[1].toLowerCase() } } }).then(function (foundUser) {
                if (foundUser == null) {
                    ...
                    if (foundUser.where == null) {
                        ...
                        self.channel.say('Last saw ' + foundUser.name + ' ' + Moment(foundUser.dateTimeSeen).fromNow() + ', in ' + foundUser.where + '\'s room');
                    }
                }
            });
