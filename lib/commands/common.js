...
c.whisperCommands = {
    ...
    'live': {
        ...
        method: function (match, user, message) {
            ...
            if (streamStrings.length == 0) {
                this.client.whisper(user, null, "No Awesome Channels currently live. :(");
                return;
            }
            
            streamStrings = streamStrings.join(", ");
            this.client.whisper(user, null, "Awesome Channels currently live: " + streamStrings);
        },
    },	
    'help': {
        time: 30,
        regex: /^h[aeuioy]lp$/i,
        method: function (match, user, message) {
            this.client.whisper(user, null, match[0] + ": " + "http://anaerins.blogspot.ca/2015/07/legbot.html");
        }
    },
    ...
    'calendar': {
        ...
        regex: /^calendar$/i,
        method: function (match, user, message) {
            this.client.whisper(user, null, "You can find the Fan Stream calendar at: http://bit.ly/LRRFanStreamCalendar2");
        }
    },
    ...
    'lrr': {
        ...
        regex: /^lrr$/i,
        method: function (match, user, message) {
            this.client.whisper(user, null, "For awesome video content and streams, check out: http://www.loadingreadyrun.com");
        }
    },
    ...
    'crossing': {
        ...
        method: function (match, user, message) {
            this.client.whisper(user, null, "If you're a fan streamer and want to cross the streams with other fans, then we're trying arrange things on a spreadsheet here http://bit.ly/CrossingFans Register your interest for what ever multiplayer games you might feel like.");
        }
    }
}
