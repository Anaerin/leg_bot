...
c.onCommand = function (user, command) {
    ...
    if (command.match(/^stats$/i) || command.match(/^counters$/i) || command.match(/^counter list$/i)) {
        return this.sayStatList(user);
    }
...
c.addCounter = function(user, command, single, plural) {
    if (this.checkModLoud(user)) {
        if (this.statistics[command]) {
            this.channel.whisper(user, "Counter " + command + " already exists!");
        } else {
            this.user = user;
            var me = this;
            ...
            }).then(this.sayCounterAdded.bind(this, command, user));
        }
    }
}

c.sayCounterAdded = function (command, user) {
    this.loadStatistics()
    this.channel.whisper(user, "Counter " + command + " added!");
}

c.delCounter = function (user, command) {
    if (this.checkModLoud(user)) {
        if (!this.statistics[command]) {
            this.channel.whisper(user, "There is no counter for the command " + command);
        } else {
            ...
            var me = this;
            Models.Count.destroy({ where: { StatisticId: deadId } }).then(function () {
                me.channel.whisper(user, "The counter " + command + " has been removed.");
                me.loadStatistics();
            });
        }
    }
...
c.sayStatList = function(user){
	...
	this.channel.whisper(user, "Statistics for this channel are: " + statList);
}
...
c.modifyStat = function(user, value, statistic){
	...
	var game;
	if((game = this.checkGame(user)) && this.checkModLoud(user)){
...
c.doModifyStat = function(statistic, game, value, count){
	count[0].value += value;
	count[0].save();
    this.channel.sendWS({ 'action': 'StatChanged', 'stat': statistic.command, 'value': count[0].value });
...
c.triggerStat = function(user, statistic){
	...
	var game;
	if((game = this.checkGame(user)) && this.checkTimeLoud(statistic.name + "Trigger", 5, user)){
		this.getCount(statistic, game)
