...
c.onCommand = function (user, command) {
    ...
    if (command.match(/^stats$/i)) {
        ...
        cmdFunk = this.addCounter.bind(this, match[1], match[2], match[3]);
        statName = true;
    }
    else if (match = command.match(/^counter +remove +(\w+)$/i)) {
        cmdFunk = this.delCounter.bind(this, match[1]);
        statName = match[1];
    }
...
c.addCounter = function(command, single, plural) {
