...
c.onCommand = function (user, command) {
    ...
    //If we are asked for help, give it.
    if (command.match(/^counter +help$/i)) {
        return this.sayHelp();
    }
    
    //If we are asked for a list of stats we do that and stop looking for any other commands.
    else if (command.match(/^stats$/i) || command.match(/^counters$/i) || command.match(/^counter list$/i)) {
...
c.sayHelp = function () {
    this.channel.say('Usage: !<counter> - Increment counter, !<counter> count - report current count, !<counter> add <#>, !<counter> remove <#>, !<counter> set <#>, !total <counter> - reports total for that counter, !counter add <name> "<single>" "<plural>", !counter remove <name>');
}
