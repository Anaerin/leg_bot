...
c.attachCommands = function(){
	...
	//Hats' channel has it's own bot handling quotes. So don't attach the quote command in his channel.
	if (this.hashtag != "#cantwearhats") this.attachCommand(commands.quote);
...
c.onMessage = function(user, message){
    ...
    if (this.isBotMod() && !this.isMod(user)) {
        ...
        if (matchingRules) {
			...
			this.client.clientConnection.timeout(this.model.hashtag, user.username, Math.pow(4, timeout - 1));
