...
c.sendWS = function (json) {
	if (this.websockets.length > 0) this.log.info("Websocket sending ", JSON.stringify(json), " to ",this.websockets.length, " sockets");
...
c.bindLog = function(){
	...
	this.log.debug = log.debug.bind(log, "N:", this.network, "C:", name);
}
...
c.onUserModded = function(user){
	...
	if (username == settings.user_name) this.amMod = true;
	this.log.debug("U:", username, "is now a mod");
...
c.onUserUnmodded = function(user){
    ...
    var username = user;
	if (user.username) username = user.username;
	if (username == settings.user_name) this.amMod = false;
...
c.newWebSocket = function (ws) {
	var game = this.getGame();
	if (game) {
		if (this.statistics && this.statistics.statistics) {
			var statNames = Object.keys(this.statistics.statistics);
			var statIDs = [], statNames = {};
			statNames.forEach((stat) => {
				statIDs.push(this.statistics.statistics[stat].id);
				statNames[this.statistics.statistics[stat].id] = stat;
			});
			models.Count.findAll({
				where: { StatisticID: { $in: statIDs }, game: game }
			}).then((counts) => {
				var statValues = {};
				counts.forEach((count) => {
					statValues[statNames[count.id]] = count.value;
				});
				ws.send(JSON.stringify({
					'action': 'GameChanged', 'game': game, stats: statValues
				}));
				statIDs.forEach((id) => {
					ws.send(JSON.stringify({ 'action': 'StatChanged', 'stat': statNames[id], 'value': statValues[statNames[id]] }));
				});
			});
		}
	}
    ...
    if (game) {
        if (this.statistics) {
            if (this.statistics.statistics) {
                ...
                models.Count.findAll({ where: { StatisticId: { $in: statIDs }, game: game } }).then(counts => {
                    counts.forEach(function (count) {
                        statValues[stats[count.StatisticId]] = count.value;
                    });
                    var value = { 'action': 'GameChanged', 'game': game, stats: statValues }
                    this.sendWS(value);
                });
            } else {
...
c.attachCommands = function(){
    ...
    if (this.hashtag == "#" + settings.user_name) {
...
c.isBotMod = function(){
	return this.isMod({ username: settings.user_name });
}
...
c.onRoomState = function () {
	...
	if (new Date().valueOf() > (this.client.lastUpdated + 10000)) {
		...
		this.log.debug("Got ROOMSTATE, but it's only been " + ((new Date().valueOf() - this.client.lastUpdated) / 1000) + " seconds since last query - ignoring and resetting timer.");
	}
...
c.say = function(message){
	...
	try {
		...
		this.log.error("Couldn't say %s, this.client=%s", message, JSON.stringify(this.client));
	}
}
