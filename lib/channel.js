...
c.onMessage = function(user, message){
    ...
    if (this.isBotMod() && !this.isMod(user)) {
        ...
        if (matchingRules) {
            ...
            if (antiSpamEngine.UserTimeouts[user]) timeout = antiSpamEngine.UserTimeouts[user];
            //Increment the timeout.
            timeout++;
            //And store it back.
            antiSpamEngine.UserTimeouts[user] = timeout;
            //Timeout the user, and log it.
            this.say("/timeout " + user + " " + Math.pow(4, timeout - 1));
            //Report the block, with all pertinent details.
            this.say("Spam detected from " + user + ", " + timeout + " times, timed out for " + Math.pow(4, timeout - 1) + " seconds. Rule matched: " + matchingRules.name + ", contact Anaerin if this is wrong.");
